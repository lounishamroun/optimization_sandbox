...
if COMPILE_MODEL == True:
    with torch.no_grad():
        ...
        )  # will raise an error in case of graph break
...
assert torch.device(model.device) == torch.device(
    device
), f"Current device = {device} | model device = {model.device} "
...
def format_data(img_data, batch_size):
    ...
    assert pv.device == torch.device(
        device
    ), f"Inputs on wrong device: {pv.device} vs {device}"
...
def bench(batch_size, inputs, iterations=200, warmup=30):
    ...
    if CUDA_EVENT_MODE == True:
        assert (
            PROFILE_MODE == False
        ), "Please disable profile mode if you want to use Cuda Events mode"
    ...
    if PROFILE_MODE == True:
        assert (
            CUDA_EVENT_MODE == False
        ), "Please disable Cuda Event Mode if you want to use Cuda Events mode"
        if COMPILE_MODEL == False:
...
if __name__ == "__main__":
    ...
    if CUDA_EVENT_MODE == True:
        print(
            f"State :{precision_status} | {memory_format_status} | {model_compilation_status}"
        )
        for b in [1, 8, 32]:
